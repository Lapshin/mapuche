...
_ROW_LABEL_COLLUMN_NUMBER = -1
_ROW_EXPAND_COLLUMN_NUMBER = -2
...
class Row:
    ...
    expanded: Expand | None = Expand.no_childs
    expand_level: int | None = 0
    tree: dict | None = field(default_factory=dict)

@dataclass
class RowTree:
    """Metadata for a row relations in the DataTable."""
...
class DataTable(ScrollView, Generic[CellType], can_focus=True):
    ...
    def __init__(
        ...
        *,
        ...
        self.rows_tree: dict[RowKey, RowTree] = {}
        """Metadata about the rows nesting in the table, indexed by their key."""
        ...
        self._expand_column_key = ColumnKey()
        """The column containing row expand labels is not part of the data. This key identifies it."""
        ...
        self._expand_column = Column(self._expand_column_key, Text(), auto_width=True)
        """The largest content width out of all row expand labels in the table."""
        self._extended_row_exists = False
        """Whether or not the user has supplied any rows with expand labels."""
    ...
    @property
    def row_count_to_show(self) -> int:
        """The number of rows to show to user"""
    ...
    def watch_show_row_labels(self, show: bool) -> None:
        width, height = self.virtual_size
        column_width = self._label_column.get_render_width(self) + self._expand_column.get_render_width(self)
    ...
    @property
    def _row_expand_column_width(self) -> int:
        """The render width of the column containing row expand labels"""
        return (
            self._expand_column.get_render_width(self)
    ...
    def _update_dimensions(self, new_rows: Iterable[RowKey]) -> None:
        ...
        for row_key in new_rows:
            ...
            row_label, row_expand, cells_in_row = self._get_row_renderables(row_index)
            ...
            expand_content_width = measure(console, row_expand, 1) if row_expand else 0
            self._expand_column.content_width = max(
                self._expand_column.content_width, expand_content_width
            )
        ...
        total_width = data_cells_width + self._row_label_column_width + self._row_expand_column_width
    ...
    def add_row_tree(
        ...
        if parrent is None:
            self.rows_tree[row_tree.key] = row_tree
            self.rows_tree[row_tree.key].expanded = Expand.no_childs
            self.rows_tree[row_tree.key].expand_level = 0
            return

        def add_row_tree_recurse(
            rows_tree: dict[RowKey, RowTree],
            parrent: RowKey,
            row_tree: RowTree,
            level: int,
        ) -> int:
            level += 1
            for _, rt in rows_tree.items():
                if rt.key == parrent:
                    rt.tree[row_tree.key] = row_tree
                    self.rows[parrent].tree = rt.tree
                    return level
                l = add_row_tree_recurse(rt.tree, parrent, row_tree, level)
                if l != -1:
                    return l
            return -1
        expand_level = add_row_tree_recurse(self.rows_tree, parrent, row_tree, 0)
        if expand_level != -1:
            self.rows[parrent].expanded = Expand.collapsed
            self.rows[row_tree.key].expanded = Expand.no_childs
            self.rows[row_tree.key].expand_level = expand_level
            self._extended_row_exists = True
        else:
            raise RowDoesNotExist(f"Row key {parrent!r} is not valid.")
    ...
    def add_row(
        ...
        *cells: CellType,
        ...
        self.add_row_tree(parrent, RowTree(row_key))
    ...
    def add_rows(
        ...
        for row in rows:
            if isinstance(row, list):
                row_keys = row_keys + self.add_rows(row, parrent=row_key)
                continue
            row_key = self.add_row(*row, parrent=parrent)
    ...
    def ordered_rows(self) -> list[Row]:
        ...
        def rows_tree_to_ordered_rows(rows_tree):
            ordered_rows = []
        ...
        if cache_key in self._ordered_row_cache:
            return self._ordered_row_cache[cache_key]

        ordered_rows = rows_tree_to_ordered_rows(self.rows_tree)
        self._ordered_row_cache[cache_key] = ordered_rows
        self._row_locations = TwoWayDict({r.key: i for i, r in enumerate(ordered_rows)})
        return ordered_rows
    ...
    def _get_row_renderables(self, row_index: int) -> RowRenderables:
        ...
        if row_index == -1:
            ...
            return RowRenderables(None, None, header_row)
        ...
        return RowRenderables(label, expand, formatted_row_cells)
    ...
    def _render_cell(
        ...
        is_row_label_cell = column_index == _ROW_LABEL_COLLUMN_NUMBER
        is_row_expand_cell = column_index == _ROW_EXPAND_COLLUMN_NUMBER

        is_fixed_style_cell = (
            not is_header_cell
            and not is_row_label_cell
            and not is_row_expand_cell
        ...
        column_key = self._column_locations.get_key(column_index) or column_index
        ...
        if cell_cache_key not in self._cell_render_cache:
            ...
            row_label, row_expand, row_cells = self._get_row_renderables(row_index)

            if is_row_label_cell:
                cell = row_label if row_label is not None else ""
            elif is_row_expand_cell:
                cell = row_expand if row_expand is not None else ""
            else:
    ...
    def _get_styles_to_render_cell(
        ...
        if hover and show_cursor and show_hover_cursor:
            ...
            if is_header_cell or is_row_label_cell or is_row_expand_cell:
    ...
    def _render_line_in_row(
        ...
        if self._extended_row_exists:
            ...
            cell_location = Coordinate(row_index, _ROW_EXPAND_COLLUMN_NUMBER)
            label_cell_lines = render_cell(
                row_index,
                _ROW_EXPAND_COLLUMN_NUMBER,
                header_style,
                width=self._row_expand_column_width,
        ...
        if self._labelled_row_exists and self.show_row_labels:
            # The width of the row label is updated again on idle
            cell_location = Coordinate(row_index, _ROW_LABEL_COLLUMN_NUMBER)
            label_cell_lines = render_cell(
                row_index,
                _ROW_LABEL_COLLUMN_NUMBER,
                header_style,
    ...
    async def _on_click(self, event: events.Click) -> None:
        ...
        is_row_label_click = self.show_row_labels and column_index == _ROW_LABEL_COLLUMN_NUMBER
        is_expand_label_click = self.show_row_labels and column_index == _ROW_EXPAND_COLLUMN_NUMBER
        if is_header_click:
